...
proof; all that is really needed is $\tauimp + \tauexp > 0$, i.e.\ $\pexp < \pimp$. The other
two are not needed for this result. The market price $p_t$ cancels in
$\pimp - \pexp = \tauimp + \tauexp$, so its sign plays no role. The assumption
$\tauexp < p_t$ only guarantees that $\pexp > 0$, i.e.\ that exporting is actually paid;
without it the prosumer would have no incentive to export in any hour, which would rule out
simultaneous import and export anyway. The power balance
\eqref{eq:c_balance} only depends on the difference $\qimp - \qexp$, so importing and exporting
at the same time does not change what the consumer can consume. However, every kWh that is
bought and sold in the same hour costs $\pimp - \pexp = \tauimp + \tauexp > 0$: it is a pure
loss. Without tariffs this loss would be zero, and simultaneous import and export would no
longer be excluded.

Outside the model, the same conclusion is physically sensible: a household usually has a
single connection point to the grid, where in practice only the net power flows, and any
additional flow in both directions would only add losses (for instance Joule losses in the
cables).
