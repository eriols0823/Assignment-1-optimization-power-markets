...
One multiplier is associated with each of the seven constraints of the hourly problem: a free
multiplier for the equality, and a non-negative one for each inequality $g(x) \leq 0$. Since
the hourly cost is in DKK and every constraint is expressed in kWh/h, all multipliers are in
DKK/kWh. For the two-sided bounds on the load and on the PV production, a bar below the
multiplier ($\underline{\mu}$) marks the lower bound and a bar above it ($\overline{\mu}$)
marks the upper bound.
\begin{align}
...
optimal. Whether they are necessary and/or sufficient for this problem is discussed below.
...
\todo{Is it ever optimal to import and export in the same hour? Demonstrate it. Hint:
complementary slackness on $\muimp$ and $\muexp$ says what happens to them when
$\qimp > 0$ and $\qexp > 0$; then compare with $\pexp \leq \lbal \leq \pimp$. The brief asks
which of the conditions on the price structure stated in the problem statement (positive
tariffs, $\tauexp < p_t$) ensures the result, and to interpret it.}
