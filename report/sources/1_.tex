...
Even with all the PV production, the load cannot be covered: ${\pvprod}^* \leq \PVmax < \Lmin
\leq {\load}^*$. The balance \eqref{eq:c_balance} gives
\begin{equation}
    {\qimp}^* - {\qexp}^* = {\load}^* - {\pvprod}^* \geq \Lmin - \PVmax > 0,
\end{equation}
so in every hour the consumer imports, ${\qimp}^* > 0$ and ${\qexp}^* = 0$, and
${\lbal}^* = \pimp$. The export price $\pexp$ plays no role.
\begin{itemize}
    \item \textbf{Load:} by Step 1 of (ii), ${\load}^* = \Lmax$ if $\uL \geq \pimp$ and
    ${\load}^* = \Lmin$ if $\uL \leq \pimp$. The intermediate regime disappears: the load
    cannot follow the PV production, since ${\load}^* = \PVmax$ would violate $\load \geq \Lmin$.
    \item \textbf{PV:} by Step 1 of (iii), there is no curtailment if $\cpv < \pimp$, with
    ${\qimp}^* = {\load}^* - \PVmax$, and full curtailment if $\cpv > \pimp$, with
    ${\qimp}^* = {\load}^*$.
\end{itemize}
Compared with (ii) and (iii):
\begin{itemize}
    \item $\uL \leq \pexp$: the load is still $\Lmin$ and the PV is still fully used. Only the
    grid changes: it imports $\Lmin - \PVmax$ instead of exporting.
    \item $\pexp < \uL < \pimp$: the load changes from $\PVmax$ to $\Lmin$. The PV is still
    fully used, since $\cpv < \uL < \pimp$, and the difference is imported.
    \item $\uL \geq \pimp$: nothing changes.
\end{itemize}

\paragraph{Case $\PVmax > \Lmax$.}
Now the PV capacity exceeds the maximum load, $\PVmax > \Lmax \geq {\load}^*$. In case A the
only prior restriction is $\cpv < \uL$, so $\cpv$ can fall in any of the three ranges defined
by the two effective prices.
\begin{itemize}
    \item \textbf{Range A: $\cpv < \pexp$.} Selling PV production always leaves a positive
    margin, so there is no curtailment, ${\pvprod}^* = \PVmax$. Since
    $\PVmax > \Lmax \geq {\load}^*$, there is a surplus that is exported,
    ${\qexp}^* = \PVmax - {\load}^* > 0$ and ${\qimp}^* = 0$, so ${\lbal}^* = \pexp$. The load
    compares $\uL$ with ${\lbal}^* = \pexp$: it is $\Lmin$ if $\uL \leq \pexp$ (selling a kWh
    pays more than consuming it) and $\Lmax$ if $\uL > \pexp$ (consuming it is worth more than
    selling it). The intermediate regime ${\load}^* = \PVmax$ cannot exist, since it would
    exceed $\Lmax$.
    \item \textbf{Range B: $\pexp < \cpv < \pimp$.} Producing for export would lose money, but
    producing to supply the load is profitable, since $\cpv < \uL$ and $\cpv < \pimp$. The PV
    production supplies exactly the load and the rest is curtailed,
    ${\pvprod}^* = {\load}^*$ (partial curtailment, $\PVmax - {\load}^* > 0$), and there is no
    exchange with the grid. Partial curtailment gives ${\lbal}^* = \cpv$
    (${\muPVlo}^* = {\muPVup}^* = 0$). Then ${\muLup}^* = \uL - {\lbal}^* = \uL - \cpv > 0$,
    which forces ${\load}^* = \Lmax$.
    \item \textbf{Range C: $\cpv > \pimp$.} The grid is cheaper than the PV, so the PV is
    switched off, ${\pvprod}^* = 0$ (full curtailment), and the whole load is imported,
    ${\qimp}^* = {\load}^*$, so ${\lbal}^* = \pimp$. This range requires
    $\pimp < \cpv < \uL$, so $\uL > {\lbal}^*$ and the load is at its limit,
    ${\load}^* = \Lmax$ and ${\qimp}^* = \Lmax$.
\end{itemize}
